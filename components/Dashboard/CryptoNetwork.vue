          ...
          <form method="POST" class="w-full">
            ...
            <div class="space-y-4">
              ...
              <div>
                ...
                <div class="mt-2.5">
                  <input
                    type="text"
                    name=""
                    v-model="dform.system"
                    id=""
                    readonly
                    placeholder="Network System"
                    class="block w-full px-4 py-4 text-gray-900 placeholder-gray-600 bg-white border border-gray-400 rounded-mdfocus:border-gray-900 focus:ring-gray-900 caret-gray-900"
                  />
                </div>
              </div>
              ...
              <div class="q-pa-md">
                <q-toggle
                  @update:model-value="
                    (value, evt) => (dform.isActive = value ? 1 : 0)
                  "
                  :label="dform.isActive ? 'Active' : 'Inactive'"
                  v-model="currActive"
                />
              </div>
              <q-btn
...
export default {
  ...
  methods: {
    async GetNetworks() {
      try {
        ...
        const res = await axios.get(
          `/api/admin/services?id=${userCred.uid}&service=CRYPTONETWORKS`,
          {
            headers: {
    ...
    async SelectRow(evt, row, index) {
      this.dform = row;
      this.currActive = this.dform.isActive ? true : false;
    ...
    async SaveChanges() {
      try {
        ...
        const res = await axios.post(
          `/api/admin/services?id=${userCred.uid}&services=CRYPTONETWORKS`,
          this.dform,
          {
            headers: {
              Accept: "application/json",
